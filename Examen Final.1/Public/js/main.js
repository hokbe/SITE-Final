document.addEventListener('DOMContentLoaded', () => {
  ...
  // === Ajout au Panier via AJAX avec Confirmation Popup ===
  document.querySelectorAll('.add-to-cart').forEach(button => {
    button.addEventListener('click', function(event) {
      event.preventDefault();
      const productId = this.getAttribute('data-product-id');
      const quantity = 1; // ou récupérer une quantité si nécessaire
  ...
  // === Mise à jour de la Quantité du Panier via AJAX ===
  ...
  // === Suppression d'une Commande dans le Dashboard via AJAX ===
